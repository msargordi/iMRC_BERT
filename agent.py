...
sequence_length_pre = load_config()['model']['sequence_length']


# Added
def model_input_preproc(quest_list, memory, sequence_length=sequence_length_pre):
    """ This function outputs the encodings """
    # if config['general']['use_cuda']:
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    out = {"input_ids": torch.zeros(len(quest_list), sequence_length, dtype=torch.int32).to(device),
           "attention_mask": torch.zeros(len(quest_list), sequence_length, dtype=torch.int32).to(device),
           "token_type_ids": torch.zeros(len(quest_list), sequence_length, dtype=torch.int32).to(device)}

    def flatten(list_of_list):
        # return [item for sublist in list_of_list for item in sublist]
        return [item for sublist in list_of_list for item in
                (sublist if isinstance(sublist, (list, tuple)) else [sublist])]

    for i in range(len(quest_list)):
        # embed()
        temp_memory = flatten(memory[i])
        # temp_memory = memory[i]
        pad = sequence_length - len(temp_memory) - len(quest_list[i]) - 3
        if pad >= 0:
            out["input_ids"][i] = torch.tensor([101] + quest_list[i] + [102] + temp_memory + [102] + pad * [0],
                                               dtype=torch.int32).to(device)
            out["attention_mask"][i] = torch.tensor((sequence_length - pad) * [1] + pad * [0], dtype=torch.int32)
        else:
            out["input_ids"][i] = torch.tensor(
                ([101] + quest_list[i] + [102] + temp_memory[-(sequence_length - len(quest_list[i]) - 3):]) + [102],
                dtype=torch.int32).to(device)
            out["attention_mask"][i] = torch.tensor(sequence_length * [1], dtype=torch.int32).to(device)
        out["token_type_ids"][i] = torch.tensor(
            (len(quest_list[i]) + 2) * [0] + (sequence_length - len(quest_list[i]) - 2) * [1], dtype=torch.int32).to(
            device)
    return out
...
class CustomAgent:
    def __init__(self, config, has_token_set):
        ...
        if self.config["model"]["model"] == "bert_base":
            self.bert_tokenizer = BertTokenizer.from_pretrained(
                '/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-base-uncased')
            bert_model = BertModel.from_pretrained(
                '/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-base-uncased')
        elif self.config["model"]["model"] == "bert_base_fine_tuned":
            self.bert_tokenizer = BertTokenizer.from_pretrained(
                '/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-base-uncased-fine-tuned')
            bert_model = BertModel.from_pretrained(
                '/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-base-uncased-fine-tuned')
        elif self.config["model"]["model"] == "bert_large":
            self.bert_tokenizer = BertTokenizer.from_pretrained(
                '/home/maziar/projects/def-bengioy/maziar/imrc_storage/bert_tokenizer/')
            bert_model = BertModel.from_pretrained('/home/maziar/projects/def-bengioy/maziar/imrc_storage/bert_model/')

        elif self.config["model"]["model"] == "roberta_base":
            self.bert_tokenizer = RobertaTokenizer.from_pretrained(
                '/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/roberta-base')
        ...
        if self.config["model"]["model"][:4] == "bert":
            self.special_token_ids = [self.word2id["[PAD]"], self.word2id["[UNK]"], self.word2id["[CLS]"],
                                      self.word2id["[SEP]"], self.word2id["[MASK]"]]
        elif self.config["model"]["model"][:4] == "robe":
            self.special_token_ids = [self.word2id["<pad>"], self.word2id["<unk>"], self.word2id["<s>"],
                                      self.word2id["</s>"]]
        self.online_net = LSTM_DQN(config=self.config, bert_model=bert_model, word_vocab=self.word_vocab,
                                   action_space_size=len(self.id2action))
        self.target_net = LSTM_DQN(config=self.config, bert_model=target_bert_model, word_vocab=self.word_vocab,
                                   action_space_size=len(self.id2action))
        self.pretrained_graph_generation_net = LSTM_DQN(config=self.config, bert_model=target_bert_model,
        ...
        self.ctrlf_indices = None  # added by Maziar
        # if self.config['model']['pretrained'] is not None:
        #     self.previous_dynamics = None
    ...
    def load_config(self):
        ...
        self.qa_replay_memory = memory.QAReplayMemory(self.config['replay']['replay_memory_capacity'],
                                                      priority_fraction=self.config['replay'][
                                                          'qa_replay_memory_priority_fraction'])
        ...
        try:
            ...
            try:
                ...
                pretrained_dict = {k: v for k, v in pretrained_dict.items() if
                                   (k in model_dict and "action_scorer" not in k)}
                ...
                print(
                    "WARNING... Model dict is different with pretrained dict. I'm loading only the parameters with same labels now. Make sure you really want this...")
    ...
    def init(self, obs, infos):
        ...
        self.prev_step_is_still_interacting = np.ones((len(obs),),
                                                      dtype="float32")  # 1s and starts to be 0 when previous action is "stop"
    ...
    def get_agent_inputs(self, token_id_list, add_special_token=False):
        ...
        input_sentence = to_pt(pad_sequences(input_ids, maxlen=max(max_len(input_ids) + 3, 7)),
    ...
    def generate_commands(self, action_indices, ctrlf_indices):
        ...
        for i in range(batch_size):
            ...
            if which == self.action2id["ctrl+f"]:
                ...
                if self.config["model"]["model"] == "roberta_base":
                    # res_str.append("ctrl+f" + self.bert_tokenizer.decode([which_word]))
                    res_str.append("ctrl+f" + str(which_word))
                else:
    ...
    def choose_maxQ_command(self, action_rank, word_mask=None):
        ...
        if word_mask is not None:
            ...
            assert word_mask.shape == action_rank.shape, (word_mask.shape, action_rank.shape)
    ...
    def get_relation_features_cooccur_graph(self, relation_token_id_list, use_model):
        ...
        for i in range(num_batch):
            ...
            relation_features_sequence = model.representation_generator(input_words,
                                                                        input_mask)  # fake_batch x sent_length x hid
    ...
    def get_node_features_srl_graph(self, node_token_ids, use_model):
        ...
        for i in range(num_batch):
            ss = inputs[i * fake_batch_size: (i + 1) * fake_batch_size]
            input_words, input_mask, _ = self.get_agent_inputs(ss, add_special_token=False)
            node_features_sequence = model.representation_generator(input_words,
                                                                    input_mask)  # fake_batch x sent_length x hid
            _mask = torch.sum(input_mask, -1)  # fake_batch
            tmp = torch.eq(_mask, 0).float()
    ...
    def get_gcn_input_features(self, node_vocab, relation_vocab, use_model):
        ...
        if self.enable_graph_input == "srl":
            relation_mask = torch.autograd.Variable(
                torch.ones(node_mask.size(0), self.num_relations))  # batch x n_relation
        else:
    ...
    def get_graph_representations(self, node_features, node_mask, relation_features, relation_mask,
                                  input_adjacency_matrices, use_model):
        model = self.get_model(use_model)
        # nodes
        ...
        # r-gcn
        node_encoding_sequence = model.rgcns(node_embeddings, rgcn_relation_input,
                                             input_adjacency_matrices)  # batch x num_node x hid
    ...
    def get_match_representations(self, input_description, input_description_mask, input_quest, input_quest_mask,
                                  node_representations, node_mask, node_vocabulary, use_model):
        model = self.get_model(use_model)
        ...
        if self.enable_graph_input in ["cooccur", "relative_position"]:
            observable_node_mask = self.kg.get_observable_node_mask(input_description, question_id_matrix=input_quest,
                                                                    node_vocabulary=node_vocabulary)  # batch x num_node
    ...
    def get_ranks(self, input_description, input_description_mask, input_quest, input_quest_mask, ctrlf_word_mask,
                  node_representations, node_mask, node_vocabulary, previous_dynamics, use_model):
        """
        Given input description tensor, and previous hidden and cell states, call model forward, to get Q values of words.
        """
        # if previous_dynamics==None : print("ooh the input")
        model = self.get_model(use_model)
        match_representation_sequence = self.get_match_representations(input_description, input_description_mask,
                                                                       input_quest, input_quest_mask,
                                                                       node_representations, node_mask, node_vocabulary,
                                                                       use_model=use_model)
        if not self.recurrent:
            previous_dynamics = None
        # if self.config['model']['pretrained'] == 'Bert':
        #     previous_dynamics = self.previous_dynamics
        action_rank, ctrlf_rank, current_dynamics = model.action_scorer(match_representation_sequence,
                                                                        input_description_mask, ctrlf_word_mask,
                                                                        previous_dynamics)
        if not self.recurrent:
            current_dynamics = None
        return action_rank, ctrlf_rank, current_dynamics

    def act_greedy(self, obs, infos, input_quest, input_quest_mask, quest_id_list, previous_commands, previous_dynamics,
                   previous_belief):
        with torch.no_grad():
            batch_size = len(obs)
            ...
            if self.not_finished_yet is None:
                ...
                for i in range(batch_size):
                    if self.not_finished_yet[i] == 1.0:
                        ...
                        # if self.config['model']['pretrained'] is None:
                        self.kg.push_one(i, copy.deepcopy(obs[i]), previous_commands[i])#, infos[i]["srl"]) should ve added
            ...
            if self.enable_graph_input == "gata":
                current_adjacency_matrix, current_belief, previous_adjacency_matrix = self.graph_update_during_rl(
                    input_description, input_description_mask, previous_belief)
                for i in range(batch_size):
                ...
                node_vocabulary, relation_vocabulary, graph_triplets = [None] * batch_size, [None] * batch_size, [
                    None] * batch_size
                ...
                node_features, node_mask, relation_features, relation_mask = self.get_gcn_input_features(
                    node_vocabulary, relation_vocabulary, use_model="online")
                node_representations = self.get_graph_representations(node_features, node_mask, relation_features,
                                                                      relation_mask, graph_adj,
                                                                      use_model="online")  # batch x max_n_node x hid
            else:
                graph_triplets, node_vocabulary, relation_vocabulary = [None] * batch_size, [None] * batch_size, [
                    None] * batch_size
                node_representations, node_mask, relation_mask = None, None, None

            action_rank, ctrlf_rank, current_dynamics = self.get_ranks(input_description, input_description_mask,
                                                                       input_quest, input_quest_mask, ctrlf_word_mask,
                                                                       node_representations, node_mask, node_vocabulary,
                                                                       previous_dynamics,
            ...
            # print(len(ctrlf_rank[0]))
            # print(sum(ctrlf_word_mask[0]))
            self.ctrlf_indices = ctrlf_indices  # added by Maziar
            ...
            replay_info = [description_list, action_indices.cpu(), ctrlf_indices.cpu(), node_vocabulary,
                           relation_vocabulary, graph_triplets,
                           to_pt(self.prev_step_is_still_interacting, False, "float")]
    ...
    def act_random(self, obs, infos, input_quest, input_quest_mask, quest_id_list, previous_commands, previous_dynamics,
                   previous_belief):
        with torch.no_grad():
            batch_size = len(obs)
    ...
    def act(self, obs, infos, input_quest, input_quest_mask, quest_id_list, previous_commands, previous_dynamics,
            previous_belief, random=False):

        with torch.no_grad():
            if self.mode == "eval":
                return self.act_greedy(obs, infos, input_quest, input_quest_mask, quest_id_list, previous_commands,
                                       previous_dynamics, previous_belief)
            if random:
                return self.act_random(obs, infos, input_quest, input_quest_mask, quest_id_list, previous_commands,
                                       previous_dynamics, previous_belief)
            batch_size = len(obs)
            ...
            for i in range(batch_size):
                if self.prev_actions[-1][i] == "stop":
                    self.prev_step_is_still_interacting[i] = 0.0
            # previous step is still interacting, this is because DQN requires one step extra computation
            replay_info = [description_list, chosen_indices.cpu(), chosen_ctrlf_indices.cpu(), node_vocabulary,
                           relation_vocabulary, graph_triplets,
                           to_pt(self.prev_step_is_still_interacting, False, "float")]
    ...
    def point_maxq_position(self, point_distribution, mask):
        ...
        point_distribution_np = point_distribution_np - np.min(
            point_distribution_np) + 1e-2  # minus the min value, so that all values are non-negative
    ...
    def answer_question_act(self, observation_list, quest_list, belief):
        with torch.no_grad():
            ...
            point_rank, mask = self.answer_question(observation_list, quest_list, node_vocabulary, relation_vocabulary,
                                                    graph_adj, current_belief)  # batch x time x 2
    ...
    def get_dqn_loss(self):
        ...
        next_input_observation, next_input_observation_mask, next_observation_id_list = self.get_agent_inputs(
            next_obs_list)

        if self.enable_graph_input == "gata":
            current_adjacency_matrix = self.hidden_to_adjacency_matrix(belief, batch_size=batch_size,
                                                                       use_model="graph_generator")
            node_representations, node_mask = self.encode_belief_graph(current_adjacency_matrix, use_model="online")
            graph_node_vocabulary, graph_relation_vocabulary, graph_triplets = [None] * batch_size, [
                None] * batch_size, [None] * batch_size
        elif self.enable_graph_input != "false":
            graph_adj = to_pt(
                self.kg.get_adjacency_matrix(graph_triplets, graph_node_vocabulary, graph_relation_vocabulary),
                enable_cuda=self.use_cuda, type="float")  # batch x relation x max_n_node x max_n_node
            node_features, node_mask, relation_features, relation_mask = self.get_gcn_input_features(
                graph_node_vocabulary, graph_relation_vocabulary, use_model="online")
            node_representations = self.get_graph_representations(node_features, node_mask, relation_features,
                                                                  relation_mask, graph_adj,
                                                                  use_model="online")  # batch x max_n_node x hid
        else:
        ...
        action_rank, ctrlf_rank, _ = self.get_ranks(input_observation, input_observation_mask, input_quest,
                                                    input_quest_mask, ctrlf_word_mask, node_representations, node_mask,
                                                    graph_node_vocabulary, None, use_model="online")  # batch x vocab
        # ps_a
        ...
        with torch.no_grad():
            ...
            if self.enable_graph_input == "gata":
                next_adjacency_matrix = self.hidden_to_adjacency_matrix(next_belief, batch_size=batch_size,
                                                                        use_model="graph_generator")
                next_node_representations_online, next_node_mask = self.encode_belief_graph(next_adjacency_matrix,
                                                                                            use_model="online")
                next_node_representations_target, next_node_mask = self.encode_belief_graph(next_adjacency_matrix,
                                                                                            use_model="target")
                next_graph_node_vocabulary, next_graph_relation_vocabulary, next_graph_triplets = [None] * batch_size, [
                    None] * batch_size, [None] * batch_size
            elif self.enable_graph_input != "false":
                next_graph_adj = to_pt(self.kg.get_adjacency_matrix(next_graph_triplets, next_graph_node_vocabulary,
                                                                    next_graph_relation_vocabulary),
                                       enable_cuda=self.use_cuda,
                                       type="float")  # batch x relation x max_n_node x max_n_node
                # online
                next_node_features, next_node_mask, next_relation_features, next_relation_mask = self.get_gcn_input_features(
                    next_graph_node_vocabulary, next_graph_relation_vocabulary, use_model="online")
                next_node_representations_online = self.get_graph_representations(next_node_features, next_node_mask,
                                                                                  next_relation_features,
                                                                                  next_relation_mask, next_graph_adj,
                                                                                  use_model="online")  # batch x max_n_node x hid
                # target
                next_node_features, next_node_mask, next_relation_features, next_relation_mask = self.get_gcn_input_features(
                    next_graph_node_vocabulary, next_graph_relation_vocabulary, use_model="target")
                next_node_representations_target = self.get_graph_representations(next_node_features, next_node_mask,
                                                                                  next_relation_features,
                                                                                  next_relation_mask, next_graph_adj,
                                                                                  use_model="target")  # batch x max_n_node x hid
            else:
            ...
            next_action_rank, next_ctrlf_rank, _ = self.get_ranks(next_input_observation, next_input_observation_mask,
                                                                  input_quest, input_quest_mask, next_ctrlf_word_mask,
                                                                  next_node_representations_online, next_node_mask,
                                                                  next_graph_node_vocabulary, None,
            ...
            next_action_rank, next_ctrlf_rank, _ = self.get_ranks(next_input_observation, next_input_observation_mask,
                                                                  input_quest, input_quest_mask, next_ctrlf_word_mask,
                                                                  next_node_representations_target, next_node_mask,
                                                                  next_graph_node_vocabulary, None,
            ...
            next_is_ctrlf = torch.eq(next_action_indices,
                                     float(self.action2id["ctrl+f"])).float()  # when the action is ctrl+f, batch
            next_q_value = (next_q_value_action + next_q_value_ctrlf * next_is_ctrlf) / (
    ...
    def get_drqn_loss(self):
        ...
        if len(self.replay_memory) < self.replay_batch_size:
            return None, None
        data, contains_first_step = self.replay_memory.get_batch_of_sequences(self.replay_batch_size,
        ...
        for step_no in range(self.replay_sample_history_length):
            obs, word_indices, ctrlf_indices, graph_node_vocabulary, graph_relation_vocabulary, graph_triplets, belief, reward, next_obs, next_graph_node_vocabulary, next_graph_relation_vocabulary, next_graph_triplets, next_belief, trajectory_mask = \
            seq_obs[step_no], seq_word_indices[step_no], seq_ctrlf_indices[step_no], seq_graph_node_vocabulary[step_no], \
            seq_graph_relation_vocabulary[step_no], seq_graph_triplets[step_no], seq_belief[step_no], seq_reward[
                step_no], seq_next_obs[step_no], seq_next_graph_node_vocabulary[step_no], \
            seq_next_graph_relation_vocabulary[step_no], seq_next_graph_triplets[step_no], seq_next_belief[step_no], \
            seq_trajectory_mask[step_no]
            if self.use_cuda:
            ...
            # if self.config['model']['pretrained'] is not None:
            #     prev_dynamics = model_input_preproc(quest_id_list, obs)
            # embed()
            if self.enable_graph_input == "gata":
                current_adjacency_matrix = self.hidden_to_adjacency_matrix(belief, batch_size=batch_size,
                                                                           use_model="graph_generator")
            ...
            ctrlf_word_mask, _ = self.get_word_mask(quest_id_list, observation_id_list)
            next_ctrlf_word_mask, _ = self.get_word_mask(quest_id_list, next_observation_id_list)
            action_rank, ctrlf_rank, current_dynamics = self.get_ranks(input_observation, input_observation_mask,
                                                                       input_quest, input_quest_mask, ctrlf_word_mask,
                                                                       node_representations, node_mask,
                                                                       graph_node_vocabulary, prev_dynamics,
                                                                       use_model="online")  # batch x vocab
            # ps_a
            q_value_action = ez_gather_dim_1(action_rank, word_indices).squeeze(1)  # batch
            q_value_ctrlf = ez_gather_dim_1(ctrlf_rank, ctrlf_indices).squeeze(1)  # batch
            is_ctrlf = torch.eq(word_indices,
                                float(self.action2id["ctrl+f"])).float()  # when the action is ctrl+f, batch
            q_value = (q_value_action + q_value_ctrlf * is_ctrlf) / (is_ctrlf + 1)  # masked average
            # q_value = torch.mean(torch.stack([q_value_action, q_value_ctrlf], -1), -1)
            ...
            # if self.config['model']['pretrained'] is not None:
            #     prev_dynamics = model_input_preproc(quest_id_list, next_obs)
            ...
            with torch.no_grad():
                ...
                if self.enable_graph_input == "gata":
                    ...
                    next_node_representations_target, next_node_mask = self.encode_belief_graph(next_adjacency_matrix,
                                                                                                use_model="target")
                    next_graph_node_vocabulary, next_graph_relation_vocabulary, next_graph_triplets = [
                                                                                                          None] * batch_size, [
                                                                                                          None] * batch_size, [
                                                                                                          None] * batch_size
                elif self.enable_graph_input != "false":
                    ...
                    next_node_features, next_node_mask, next_relation_features, next_relation_mask = self.get_gcn_input_features(
                        next_graph_node_vocabulary, next_graph_relation_vocabulary, use_model="online")
                    next_node_representations_online = self.get_graph_representations(next_node_features,
                                                                                      next_node_mask,
                                                                                      next_relation_features,
                                                                                      next_relation_mask,
                                                                                      next_graph_adj,
                    ...
                    next_node_features, next_node_mask, next_relation_features, next_relation_mask = self.get_gcn_input_features(
                        next_graph_node_vocabulary, next_graph_relation_vocabulary, use_model="target")
                    next_node_representations_target = self.get_graph_representations(next_node_features,
                                                                                      next_node_mask,
                                                                                      next_relation_features,
                                                                                      next_relation_mask,
                                                                                      next_graph_adj,
                ...
                # pns Probabilities p(s_t+n, ·; θonline)
                next_action_rank, next_ctrlf_rank, _ = self.get_ranks(next_input_observation,
                                                                      next_input_observation_mask, input_quest,
                                                                      input_quest_mask, next_ctrlf_word_mask,
                                                                      next_node_representations_online, next_node_mask,
                                                                      next_graph_node_vocabulary, prev_dynamics,
                                                                      use_model="online")  # batch x vocab
                # Perform argmax action selection using online network: argmax_a[(z, p(s_t+n, a; θonline))]
                next_action_indices = self.choose_maxQ_command(next_action_rank)  # batch x 1
                next_ctrlf_indices = self.choose_maxQ_command(next_ctrlf_rank, next_ctrlf_word_mask)  # batch x 1
    ...
    def get_qa_loss(self):
        ...
        graph_node_vocabulary = [v for pos, v in zip(groundtruth_answer_positions, batch.graph_node_vocabulary) if
                                 pos is not None]
        graph_relation_vocabulary = [v for pos, v in zip(groundtruth_answer_positions, batch.graph_relation_vocabulary)
                                     if pos is not None]
        belief = [item for pos, item in zip(groundtruth_answer_positions, batch.belief) if pos is not None]
        groundtruth_answer_positions = [item for item in groundtruth_answer_positions if
                                        item is not None]  # list: batch x 2
        ...
        if self.enable_graph_input == "gata":
            ...
            graph_adj = to_pt(
                self.kg.get_adjacency_matrix(graph_triplets, graph_node_vocabulary, graph_relation_vocabulary),
                enable_cuda=self.use_cuda, type="float")  # batch x max_n_node x max_n_node
        else:
            graph_node_vocabulary, graph_relation_vocabulary, graph_adj = None, None, None

        answer_distribution, obs_mask = self.answer_question(batch_observation, quest_list, graph_node_vocabulary,
                                                             graph_relation_vocabulary, graph_adj,
                                                             belief)  # answer_distribution is batch x time x 2
    ...
    def answer_question(self, observation_list, quest_list, graph_node_vocabulary, graph_relation_vocabulary, graph_adj,
                        current_belief):
        ...
        if self.enable_graph_input == "gata":
            current_adjacency_matrix = self.hidden_to_adjacency_matrix(current_belief, batch_size=batch_size,
                                                                       use_model="graph_generator")
            node_representations, node_mask = self.encode_belief_graph(current_adjacency_matrix, use_model="online")
        ...
        matching_representation_sequence = self.get_match_representations(input_observation, input_observation_mask,
                                                                          input_quest, input_quest_mask,
                                                                          node_representations, node_mask,
                                                                          graph_node_vocabulary, use_model="online")
        # get mask
    ...
    def finish_of_episode(self, episode_no, batch_no, batch_size):
        ...
        if (
                episode_no + batch_size) % self.target_net_update_frequency <= episode_no % self.target_net_update_frequency:
            self.update_target_net()
    ...
    def encode_belief_graph(self, input_adjacency_matrices, use_model):
        ...
        # r-gcn
        node_encoding_sequence = model.rgcns(node_embeddings, relation_embeddings,
                                             input_adjacency_matrices)  # batch x num_node x hid
        node_encoding_sequence = model.graph_representation_prj(node_encoding_sequence)
    ...
    def hidden_to_adjacency_matrix(self, hidden, batch_size, use_model):
        ...
        if hidden is None:
            ...
            adjacency_matrix = adjacency_matrix.view(batch_size, int(self.num_relations / 2), self.num_nodes,
                                                     self.num_nodes)
    ...
    def graph_update_during_rl(self, input_description, input_description_mask, h_t_minus_one):
        ...
        # encode
        description_representation_sequence = self.pretrained_graph_generation_net.representation_generator(
            input_description, input_description_mask)
        # decode graph from history
        prev_adjacency_matrix = self.hidden_to_adjacency_matrix(h_t_minus_one, batch_size=batch_size,
                                                                use_model="graph_generator")
        ...
        # co-attention
        h_og = self.pretrained_graph_generation_net.belief_attention(description_representation_sequence,
                                                                     node_encoding_sequence, input_description_mask,
                                                                     node_mask)
        h_go = self.pretrained_graph_generation_net.belief_attention(node_encoding_sequence,
                                                                     description_representation_sequence, node_mask,
                                                                     input_description_mask)
        ...
        rnn_input = self.pretrained_graph_generation_net.belief_attention_to_rnn_input(
            torch.cat([ave_h_go, ave_h_og], dim=1))  # batch x hid
        rnn_input = torch.tanh(rnn_input)  # batch x hid
        h_t = self.pretrained_graph_generation_net.belief_graph_rnncell(rnn_input,
                                                                        h_t_minus_one) if h_t_minus_one is not None else self.pretrained_graph_generation_net.belief_graph_rnncell(
            rnn_input)  # both batch x hid
        current_adjacency_matrix = self.hidden_to_adjacency_matrix(h_t, batch_size=batch_size,
                                                                   use_model="graph_generator")
        return current_adjacency_matrix, h_t, prev_adjacency_matrix

    def get_observation_infomax_logits(self, batch_positive, batch_negative, h_t_minus_one, prev_adjacency_matrix=None,
                                       prev_node_encoding_sequence=None, prev_node_mask=None):
        ...
        if prev_node_encoding_sequence is None:
            if prev_adjacency_matrix is None:
                # decode graph from history
                prev_adjacency_matrix = self.hidden_to_adjacency_matrix(h_t_minus_one, batch_size=len(batch_positive),
                                                                        use_model="online")
            # encode history graph
            prev_node_encoding_sequence, prev_node_mask = self.encode_belief_graph(prev_adjacency_matrix,
                                                                                   use_model="online")

        # co-attention
        h_og = self.online_net.belief_attention(positive_representation_sequence, prev_node_encoding_sequence,
                                                positive_mask, prev_node_mask)
        h_go = self.online_net.belief_attention(prev_node_encoding_sequence, positive_representation_sequence,
                                                prev_node_mask, positive_mask)
        ...
        rnn_input = torch.tanh(rnn_input)  # batch x hid
        h_t = self.online_net.belief_graph_rnncell(rnn_input,
                                                   h_t_minus_one) if h_t_minus_one is not None else self.online_net.belief_graph_rnncell(
            rnn_input)  # both batch x hid
        current_adjacency_matrix = self.hidden_to_adjacency_matrix(h_t, batch_size=len(batch_positive),
                                                                   use_model="online")
        current_node_encoding_sequence, current_node_mask = self.encode_belief_graph(current_adjacency_matrix,
                                                                                     use_model="online")
        new_belief_graph_representations = masked_mean(current_node_encoding_sequence, m=current_node_mask,
                                                       dim=1)  # batch x hid

        logits = self.online_net.observation_discriminator(new_belief_graph_representations,
                                                           positive_representation_sequence, positive_mask,
                                                           negative_representation_sequence, negative_mask)
    ...
    def get_observation_infomax_loss(self, positive_samples, negative_samples, evaluate=False):
        ...
        for i in range(max_num_sentences):
            ...
            logits, current_graph_hidden_state, current_adjacency_matrix, current_node_encoding_sequence, current_node_mask = self.get_observation_infomax_logits(
                batch_positive, batch_negative, previous_graph_hidden_state, prev_adjacency_matrix,
                prev_node_encoding_sequence, prev_node_mask)
