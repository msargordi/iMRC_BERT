...
class GamifiedSquad(gym.Env):
    ...
    def __init__(self, config):
        ...
        self.split = "train"
        self.in_sent = 0  # added by maziar
...
#             self.data_path = os.environ['PT_DATA_DIR'] + "/" + self.data_path
    ...
    def reset(self, random=True):
        ...
        for _a, _b in zip(self.current_sentence_id_list, self.current_srl_list):
            ...
            if self.pretrained is None and self.data_path is "squad_split":
        ...
        if self.pretrained is None and self.data_path is "squad_split":
            ...
            infos = [{"q": q, "a": a, "a_string": a_string, "full_obs": full, "which": which, "srl": [], "full_srl": [], "q_srl": [], "stopped": False} for
                     q, a, a_string, full, which in
                     zip(self.current_question_id, self.current_answer_id_list, self.current_answers,
                         self.current_sentence_id_list, which_sentence)]
    ...
    def step(self, actions):
        ...
        for i in range(len(actions)):
            ...
            if actions[i] == "next":
                ...
                if self.config["model"]["model"][:4] == "robe":
                    # query = actions[i][6:]
                    # if query[0] == " ":
                    #     query = "Ġ" + query[1:]
                    # if query not in self.word2id:
                    #     query = "<unk>"
                    query_id = int(actions[i][6:])
                else:
                ...
                for j in range(1, len(self.current_sentence_id_list[i]) + 1):
                    ...
                    if query_id in sent:
                        self.in_sent += 1  # added by maziar
                        break
            else:
            ...
            if self.pretrained is None and self.data_path is "squad_split":
                ...
                infos.append({"q": self.infos[i]["q"], "a": self.infos[i]["a"], "a_string": self.infos[i]["a_string"],
                              "which": new_which, "stopped": stopped, "full_obs": self.infos[i]["full_obs"]})
