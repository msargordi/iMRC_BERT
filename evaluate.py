    ...
    with torch.no_grad():
        ...
        while (True):
            ...
            for _ in range(agent.max_nb_steps_per_episode):
                commands, replay_info, current_dynamics, current_belief = agent.act_greedy(obs, infos, input_quest,
                                                                                           input_quest_mask,
                                                                                           quest_id_list, prev_commands,
                                                                                           previous_dynamics,
                                                                                           previous_belief)
            ...
            if verbose:
                for i in range(batch_size):
                    to_print.append(str(counter) + " -------------------------------------------- ig: " + str(
                        ig_reward_np[i]) + ", qa: " + str(qa_reward_np[i]))
        ...
        if verbose:
            with open(agent.experiment_tag + "_output.txt", "w") as f:
                f.write("\n".join(to_print))
        print("===== Eval =====: qa acc: {:2.3f} | correct state: {:2.3f} | used steps: {:2.3f}".format(
            np.mean(np.array(print_qa_acc)), np.mean(np.array(print_ig_acc)), np.mean(np.array(print_steps))))
    ...
    with torch.no_grad():
        ...
        while (True):
            ...
            for _ in range(agents[0].max_nb_steps_per_episode):
                ...
                for i in range(num_agents):
                    action_rank, ctrlf_rank, ctrlf_word_mask, current_dynamics, current_belief, replay_info = agents[
                        i].get_ranks_greedy(obs, infos, input_quest, input_quest_mask, quest_id_list, prev_commands,
                                            previous_dynamics[i], previous_belief[i])
                    if i == 0:
                        tmp_replay_buffer.append(replay_info)
            ...
            for i in range(num_agents):
                qa_rank, qa_mask = agents[i].get_qa_ranks_greedy(agents[0].naozi.get(), quest_list,
                                                                 previous_belief[i])  # batch
            ...
            chosen_head_tails_np = generic.to_np(chosen_head_tails)
            chosen_answer_strings = generic.get_answer_strings(agents[0].naozi.get(), chosen_head_tails_np,
                                                               agents[0].bert_tokenizer, agents[0].special_token_ids)
