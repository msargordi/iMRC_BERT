...
from generic import load_config

if load_config()['model']['model'] == "bert_base" or "bert_base_fine_tuned" or "roberta_base":
    BERT_EMB_SIZE = 768
elif load_config()['model']['model'] == "bert_large" or "roberta_large":
    BERT_EMB_SIZE = 1024
...
#         if load_config()['model']['pretrained'] == "Bert_large_finetuned":
#             self.model = BertModel.from_pretrained(
#                 "/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-large-uncased-whole-word-masking-finetuned-squad")
#         elif load_config()['model']['pretrained'] == "Bert":
#             self.model = BertModel.from_pretrained(
#                 "/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-base-uncased")
#         elif load_config()['model']['pretrained'] == "Bert_fine_tuned":
#             self.model = BertModel.from_pretrained(
...
#         sequence_output = self.model(**encoded_input)
#         # embed()
...
class LSTM_DQN(torch.nn.Module):
    ...
    def _def_layers(self):
        ...
        # bert_model=BertModel.from_pretrained("/home/maziar/projects/def-bengioy/maziar/imrc_storage/models/bert-base-uncased")
        if self.recurrent:
            # if self.pretrained is not None:
            #     self.rnncell = CustomModel()
            # else:
    ...
    def action_scorer(self, state_representation_sequence, mask, ctrlf_word_mask, previous_dynamics):
        ...
        if self.recurrent:
            if self.pretrained is not None:
                if previous_dynamics==None: print("Yaaaaaak model")
                # embed()
                current_dynamics = self.rnncell(**previous_dynamics)  #.mean(axis=1)
            else:
                # embed()
