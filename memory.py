...
class QAReplayMemory(object):
    ...
    def sample(self, batch_size):
        if self.priority_fraction == 0.0:
            ...
            # res = self.rng.choice(np.array(self.beta_memory, dtype=object), from_beta)
            res = np.array(self.beta_memory, dtype=object)[self.rng.choice(len(self.beta_memory), from_beta)].tolist()
        else:
            from_alpha = min(int(self.priority_fraction * batch_size), len(self.alpha_memory))
            ...
            res = np.array(self.alpha_memory, dtype=object)[self.rng.choice(len(self.alpha_memory), from_alpha)].tolist() + np.array(self.beta_memory, dtype=object)[self.rng.choice(len(self.beta_memory), from_beta)].tolist()
        self.rng.shuffle(res)
        return res
