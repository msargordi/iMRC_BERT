...
if generic.load_config()["general"]["enable_graph_input"] is not "gata":
    torch.backends.cuda.matmul.allow_tf32 = True
    # The flag below controls whether to allow TF32 on cuDNN. This flag defaults to True.
    torch.backends.cudnn.allow_tf32 = True

wandb.init(project="iMRC_graph",
           entity="mazeiar6672",
           # entity="long-document-qa",
           # mode="offline",
           mode="disabled",
           config=generic.load_config(),
           id=generic.load_config()["checkpoint"]["experiment_tag"])

# sequence_length_pre = generic.load_config()['model']['sequence_length']
...
def train():
    ...
    # model = CustomAgent(config, env.has_token_set)
    # if torch.cuda.device_count() > 1:
    #     dist.init_process_group(backend='nccl')
    #     print("Using", torch.cuda.device_count(), "GPUs")
    #     agent = DDP(model, device_ids=[torch.cuda.current_device()])
    # else:
    #     agent = model
    ...
    #     output_dir = "."
    # output_dir = "/home/maziar/projects/def-bengioy/maziar/imrc_storage/results"
    output_dir = "/home/maziar/scratch/results_iMRC"
    data_dir = "."
    ...
    while True:
        ...
        report = agent.report_frequency > 0 and (
                    episode_no % agent.report_frequency <= max(episode_no - batch_size, 0) % agent.report_frequency)
        ...
        # embed()
        # if config["model"]["pretrained"] is None:  # to change . . .
        ...
        for _ in range(agent.max_nb_steps_per_episode):
            ...
            # if config["model"]["pretrained"] is not None:
                # embed()
                # previous_dynamics = model_input_preproc(quest_list, agent.naozi.get_sent_list())
                # agent.previous_dynamics = previous_dynamics
            # embed()
            commands, replay_info, current_dynamics, current_belief = agent.act(obs, infos, input_quest,
                                                                                input_quest_mask, quest_id_list,
                                                                                prev_commands, previous_dynamics,
                                                                                previous_belief, random=act_randomly)
        ...
        chosen_answer_strings = generic.get_answer_strings(agent.naozi.get(), chosen_head_tails_np,
                                                           agent.bert_tokenizer, agent.special_token_ids)
        ...
        for b in range(batch_size):  # data points in batch
            ...
            if np.mean(ig_reward_np[b]) == 0.0:
                continue
            agent.qa_replay_memory.push(is_prior, qa_reward_np[b], agent.naozi.get_sentence_lists(b), quest_list[b],
                                        replay_node_vocab[b], replay_relation_vocab[b], replay_triplets[b],
                                        answer_token_ids[b],
                                        belief_buffer[-1][b].cpu() if belief_buffer[-1][b] is not None else None)
        ...
        for b in range(len(ig_reward_np)):
            ...
            mem = []
            for i in range(len(tmp_replay_buffer)):
                batch_description_list, batch_chosen_indices, batch_chosen_ctrlf_indices, batch_graph_node_vocabulary, batch_graph_relation_vocabulary, batch_graph_triplets, _, batch_rewards = \
                tmp_replay_buffer[i]
        ...
        wandb.log({"Episode": episode_no,
                   "Interaction loss": running_avg_ig_loss.get_avg(),
                   "Interaction qvalue": running_avg_ig_q_value.get_avg(),
                   "QA loss": running_avg_qa_loss.get_avg(),
                   "Rewards": print_rewards,
                   "QA acc": qa_acc,
                   "QA acc - sufficient": qa_acc,
                   "Sufficient info 1": ig_acc,
                   "Sufficient info 2": running_avg_ig_acc.get_avg(),
                   "Used steps": running_avg_steps.get_avg()})  # added

        time_2 = datetime.datetime.now()
        eastern_time = datetime.datetime.now(timezone('US/Eastern')).strftime("%b %d %Y %H:%M:%S")
        if report:
            print(
                "Episode: {:3d} | {:s} | time spent: {:s} | interaction loss: {:2.3f} | interaction qvalue: {:2.3f} | qa loss: {:2.3f} | rewards: {:2.3f} | qa acc: {:2.3f}/{:2.3f} | sufficient info: {:2.3f}/{:2.3f} | used steps: {:2.3f}".format(
                    episode_no, eastern_time, str(time_2 - time_1).rsplit(".")[0], running_avg_ig_loss.get_avg(),
                    running_avg_ig_q_value.get_avg(), running_avg_qa_loss.get_avg(), print_rewards, qa_acc,
                    running_avg_qa_acc.get_avg(), ig_acc, running_avg_ig_acc.get_avg(), running_avg_steps.get_avg()))
        if __save__:
        ...
        if agent.run_eval:
            ...
            wandb.log({"QA evaluation acc": eval_qa_acc,
                       "eval_ig_acc": eval_ig_acc,
                       "eval_used_steps": eval_used_steps})
    ...
    # if torch.cuda.device_count() > 1:   # added
